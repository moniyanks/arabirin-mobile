...
export function getCycleConditionOverlay({
  ...
  if (lower.includes('fibroids')) {
    return {
      noticeToday:
        phase === 'period' ? ['Flow', 'Clots', 'Pain', 'Pressure'] : ['Flow', 'Pain', 'Energy'],
  ...
  if (lower.includes('pcos')) {
    return {
      ...
      heroMetaOverride: 'Predictions may vary. Focus on patterns instead of precision.',
      insightOverride:
  ...
  if (lower.includes('endometriosis')) {
    return {
      noticeToday: phase === 'period' ? ['Pain', 'Energy', 'Rest'] : ['Pain', 'Fatigue', 'Mood'],
      heroSubtitleOverride:
        'Pain and energy may shift in ways that don’t follow a typical cycle pattern.',
      heroMetaOverride: 'Your experience may vary from standard cycle expectations.',
      insightOverride:
