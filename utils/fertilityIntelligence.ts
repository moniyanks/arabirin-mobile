...
function calculateCycleQuality(
  ...
  const quality: CycleQuality =
    score >= 80 ? 'optimal' : score >= 60 ? 'good' : score >= 40 ? 'fair' : 'needs_attention'
...
export function calculateFertilityInsight(
  ...
  if (!periods.length || !fw) {
    return {
      ...
      cycleQuality: 'fair',
      cycleQualityScore: 50,
      cycleQualityFactors: [],
      lutealPhaseLength: null,
      lutealStatus: 'unknown',
      confidence: 'low',
      cyclesAnalysed: 0,
      avgCycleLength,
      cervicalMucusLogs,
  ...
  const lutealStatus: LutealStatus =
    lutealPhaseLength === null ? 'unknown' : lutealPhaseLength < 10 ? 'short' : 'normal'
  ...
  const confidence: 'low' | 'medium' | 'high' =
    metrics.totalCycles >= 5 ? 'high' : metrics.totalCycles >= 3 ? 'medium' : 'low'
...
export const FERTILE_STATUS_MESSAGES: Record<FertileWindowStatus, string> = {
  before_fertile:
    'Your fertile window is coming up. This is a good time to track symptoms closely.',
  in_fertile:
    'You are in your fertile window. This is your most important time if you are trying to conceive.',
