...
function getCycleLengths(periods: Period[]) {
  ...
  for (let i = 1; i < sorted.length; i++) {
    const diff = Math.round(
      (parseLocalDate(sorted[i].startDate).getTime() -
        parseLocalDate(sorted[i - 1].startDate).getTime()) /
        86400000
    )
...
function getPeriodLengths(periods: Period[]) {
  return periods
    ...
    .map((p) => {
      const diff =
        Math.round(
          (parseLocalDate(p.endDate!).getTime() - parseLocalDate(p.startDate).getTime()) / 86400000
        ) + 1
...
export function buildBodyMetrics(
  ...
  else if (cycleLengths.length >= RULES.MIN_CYCLES_FOR_CONFIDENCE_MEDIUM)
...
export function generateInsights(metrics: BodyMetrics): Insight[] {
  ...
  if (metrics.totalCycles < RULES.MIN_CYCLES_FOR_STABILITY) {
    ...
    metrics.cycleVariance !== null &&
    metrics.cycleVariance <= RULES.STABLE_CYCLE_VARIANCE_DAYS
  ) {
    insights.push({
      id: 'cycle_stability',
      type: 'cycle',
    ...
    insights.push({
      ...
      message:
        'Your cycle length varies more than usual. If this continues, it may be worth tracking closely.',
      confidence: metrics.predictionConfidence
    })
  }
  ...
  if (metrics.heavyFlowCount >= RULES.HEAVY_FLOW_THRESHOLD_COUNT) {
    insights.push({
      id: 'heavy_flow_pattern',
      type: 'health',
      severity: 'attention',
      title: 'Heavy Flow Pattern',
      message:
        'Heavy bleeding has appeared repeatedly in your logs. Some women with similar patterns discuss fibroids or iron levels with a clinician.',
      confidence: 'medium'
    })
  }
  ...
  if (metrics.crampsCount >= 3) {
    insights.push({
      ...
      message:
        'Cramps appear frequently in your recent logs. Tracking when they occur can help you understand your cycle rhythm.',
      confidence: 'medium'
    })
  }
