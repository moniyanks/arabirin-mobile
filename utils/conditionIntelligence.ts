...
function getWeeksOfData(logs: SymptomLog[]): number {
  ...
  return Math.floor(
    (new Date(sorted[sorted.length - 1].log_date).getTime() -
      new Date(sorted[0].log_date).getTime()) /
      (86400000 * 7)
  )
}
...
export function calculateConditionScore(
  ...
  const percentage = signals.filter((s) => s.met).reduce((sum, s) => sum + s.weight, 0)
...
export const SCORE_DESCRIPTIONS: Record<ConditionKey, Record<ScoreLevel, string>> = {
  fibroids: {
    ...
    moderate:
      'Some of your logged patterns are consistent with symptoms commonly associated with fibroids. Worth mentioning to your healthcare provider.',
    notable:
      'Several logged patterns show notable similarity to fibroid-associated profiles. We recommend discussing this with your healthcare provider.'
  },
  endo: {
    ...
    moderate:
      'Some of your logged patterns are consistent with symptoms commonly associated with endometriosis. Worth mentioning to your healthcare provider.',
    notable:
      'Several logged patterns show notable similarity to endometriosis-associated profiles. We recommend discussing this with your healthcare provider.'
  },
  pcos: {
    low: 'Your logged patterns show low similarity to PCOS-associated profiles. Continue logging to build a clearer picture.',
    moderate:
      'Some of your logged patterns are consistent with symptoms commonly associated with PCOS. Worth mentioning to your healthcare provider.',
    notable:
      'Several logged patterns show notable similarity to PCOS-associated profiles. We recommend discussing this with your healthcare provider.'
  },
  thalassemia: {
    ...
    moderate:
      'Some of your logged patterns are consistent with symptoms commonly associated with thalassemia. Worth mentioning to your healthcare provider, especially if you have not been tested.',
    notable:
      'Several logged patterns show notable similarity to thalassemia-associated profiles. We recommend asking your healthcare provider about haemoglobin electrophoresis testing.'
  }
}
